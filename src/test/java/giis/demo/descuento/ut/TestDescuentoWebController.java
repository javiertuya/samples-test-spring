 ...
 * sistema. Es similar a TestDescuentoRestService, con los siguientes cambios:
 * 
 * <br/>
 * - Como utiliza solo el controlador (el servicio que accede a esta se implementa con un mock),
 * SpringBootTest no precisa en este caso indicar la clase que definira el contexto de la aplicacion <br/>
 * - Como no se usara la base de datos no hace falta indicar TestPropertySource
 * 
 * <br/>
 * Este ejemplo ilustra solamente la forma de interactuar con el controlador de forma aislada. Estas pruebas
 * para el controlador se realizaran normalmente si este incluye una logica compleja que no sera probada
 * cuando se realicen pruebas integrando el interfaz de usuario, de lo contrario se estaria probando lo mismo
 * dos veces.
 */
@SpringBootTest
...
public class TestDescuentoWebController {
	 ...
	 * simulando el comportamiento con una base de datos con dos filas (1,18,'S','N','N'), (2,38,'S','S','N') para
	 * dos situaciones a cubrir: <br>
	...
	/**
	 * Situación (1): Get para obtener la pagina inicial, no se especifica edad, devolvera dos filas (de acuerdo
	 * con el mock). Ilustra la forma habitual de comprobar los valores que devuelve el controlador utilizando
	 * matchers para: <br>
	 * - "command" contiene el campo donde el usuario introduce la edad (es nulo al ser un get) <br>
	 * - "descuento" contiene la lista de descuentos (id,descuento) que devuelve el controlador
	 */
	@Test
	public void testGetRequest() throws Exception {
		ResultActions res = mvc.perform(get("/descuentos")).andExpect(status().isOk());
		...
		res.andExpect(model().attribute("descuentos",
				hasItem(allOf(hasProperty("id", is(1)), hasProperty("descuento", is(15))))));
		res.andExpect(model().attribute("descuentos",
				hasItem(allOf(hasProperty("id", is(2)), hasProperty("descuento", is(20))))));
	}

	/**
	 * Situacion (2): Post indicando edad 19, devolvera una fila (de acuerdo con el mock) Ilustra el paso de
	 * parametros hacia el controlador y diferntes formas de comprobar los valores que devuelve el este: <br>
	 * - "command" es el campo donde el usuario introduce la edad (en este caso, 19) <br>
	 * - "descuento" contiene la lista de descuentos (id,descuento) que devuelve el controlador <br>
	 * Ademas de usar matchers, ilustra como se determinan los objetos que el controlador envia a la vista para
	 * hacer comparaciones, y la obtencion del contenido completo html de la respuesta
	 */
	@Test
	public void testPostRequest() throws Exception {
		// en el post debe especificar el parametro del form (campos DescuentoFormDTO)
		ResultActions res = mvc.perform(post("/descuentos").param("edad", "19")).andExpect(status().isOk());
