...
/**
 * Controlador web para la funcionalidad de visualizacion descuentos de clientes. Define los endpoints que
 * invoca la vista y ejecuta la logica de negocio correspondiente. La comunicacion con la vista se realiza
 * mediante un objeto Model que contiene una serie de entradas con los objetos que se comunican hacia la vista
 * (descuentos y clientes) y con los que se reciben desde la vista en el post (command)
 */
@Controller
public class DescuentoWebController {
	...
	/**
	 * Pagina resultado de un postback: recibe command con la edad del cliente a usar como filtro y pone los
	 * valores correspondientes a los objetos que se veran en la vista. Ver
	 * https://medium.com/@grokwich/spring-boot-thymeleaf-html-form-handling-762ef0d51327 para las diferentes
	 * formas en que se mapean los campos de un form a objetos
	 */
	@PostMapping("/" + DESCUENTOS_TEMPLATE)
	public String descuentosSubmit(@Valid @ModelAttribute(DESCUENTOS_FORM) DescuentoFormDTO command,
